...
func TestQuicTransport_WriteTo_ShouldNotLogAsWarningExpectedFailures(t *testing.T) {
	tests := map[string]struct {
		setup          func(t *testing.T, cfg *QuicTransportConfig)
		remoteAddr     string
		expectedLogs   string
		...
		"should log 'connection refused' if debug log level is enabled": {
			setup: func(_ *testing.T, cfg *QuicTransportConfig) {
	...
	for testName, testData := range tests {
		t.Run(testName, func(t *testing.T) {
			...
			transport, err := NewQuicTransport(cfg, logger, nil)
...
func TestQuicFinalAdvertiseAddr(t *testing.T) {
	tests := map[string]struct {
	...
	for testName, testData := range tests {
		t.Run(testName, func(t *testing.T) {
			...
			transport, err := NewQuicTransport(cfg, logger, prometheus.NewPedanticRegistry())
			require.NoError(t, err)
...
func TestQuicNonIPsAreRejected(t *testing.T) {
	cfg := QuicTransportConfig{BindAddrs: flagext.StringSlice{"localhost"}}
	_, err := NewQuicTransport(cfg, nil, nil)
