	...
	for testName, testData := range tests {
		t.Run(testName, func(t *testing.T) {
			req := &PrometheusRequest{
				Start: timeutil.TimeToMillis(testData.reqStartTime),
				End:   timeutil.TimeToMillis(testData.reqEndTime),
			}
			...
			if testData.expectedSkipped {
				...
				assert.InDelta(t, timeutil.TimeToMillis(testData.expectedStartTime), inner.Calls[0].Arguments.Get(1).(Request).GetStart(), delta)
				assert.InDelta(t, timeutil.TimeToMillis(testData.expectedEndTime), inner.Calls[0].Arguments.Get(1).(Request).GetEnd(), delta)
			}
		})
	}
}
	...
	for testName, testData := range tests {
		t.Run(testName, func(t *testing.T) {
			...
			if testData.expectedErr != "" {
				...
				require.Len(t, inner.Calls, 1)
				assert.Equal(t, timeutil.TimeToMillis(testData.reqStartTime), inner.Calls[0].Arguments.Get(1).(Request).GetStart())
				assert.Equal(t, timeutil.TimeToMillis(testData.reqEndTime), inner.Calls[0].Arguments.Get(1).(Request).GetEnd())
			}
		})
	}
}
