...
func TestCompareMatrix(t *testing.T) {
	for _, tc := range []struct {
		...
		t.Run(tc.name, func(t *testing.T) {
			err := compareMatrix(tc.expected, tc.actual, 0, log.NewNopLogger())
			if tc.err == nil {
...
func TestCompareVector(t *testing.T) {
	for _, tc := range []struct {
		...
		t.Run(tc.name, func(t *testing.T) {
			err := compareVector(tc.expected, tc.actual, 0, log.NewNopLogger())
			if tc.err == nil {
				require.NoError(t, err)
				return
			}
...
func TestCompareScalar(t *testing.T) {
	for _, tc := range []struct {
		...
		t.Run(tc.name, func(t *testing.T) {
			err := compareScalar(tc.expected, tc.actual, 0, log.NewNopLogger())
			if tc.err == nil {
				require.NoError(t, err)
				return
			}
...
func TestCompareSamplesResponse(t *testing.T) {
	for _, tc := range []struct {
		...
		t.Run(tc.name, func(t *testing.T) {
			samplesComparator := NewSamplesComparator(tc.tolerance, log.NewNopLogger())
